...
/**
 * Contract source code samples for AI audit.
 * In production, these would be fetched from a block explorer API.
 * Maps contract address → Solidity source code.
 */
export const CONTRACT_SOURCES: Record<string, string> = {
    "0x742d35Cc6634C0532925a3b844Bc9e7595f2bD28": `// SPDX-License-Identifier: MIT
...
contract PurpleToken is ERC20 {
    ...
    "0x8B3a08B22F23f37FA4e2E0E5a0F147F4829E2c3A": `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
contract SimpleDEXPool {
    ...
    "0xDEAD000000000000000000000000000000001337": `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
contract HoneypotToken {
    ...
    function sell(uint256 amount) external {
        require(sellEnabled, "Trading not enabled yet");
        require(!_blacklisted[msg.sender], "Blacklisted");
        uint256 taxed = amount * (100 - _hiddenTax) / 100;
        balanceOf[msg.sender] -= amount;
        payable(msg.sender).transfer(taxed);
    }
    function enableSelling() external onlyOwner { sellEnabled = true; }
    function setTax(uint256 tax) external onlyOwner { _hiddenTax = tax; }
    function blacklist(address user) external onlyOwner { _blacklisted[user] = true; }
    function withdrawAll() external onlyOwner { payable(owner).transfer(address(this).balance); }
}`,

    "0xBAD0000000000000000000000000000000000069": `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
contract RugPullDex {
    address public owner;
    ...
    constructor() { owner = msg.sender; }
    modifier onlyOwner() { require(msg.sender == owner); _; }
    function addLiquidity() external payable { liquidity[msg.sender] += msg.value; }
    function removeLiquidity(uint256 amount) external {
        require(msg.sender == owner, "Only owner");
        payable(owner).transfer(amount);
    }
    function pause() external onlyOwner { paused = true; }
    function swap(address, uint256) external view { require(!paused, "Paused"); }
    function emergencyWithdraw() external onlyOwner { selfdestruct(payable(owner)); }
}`,

    "0xFAKE00000000000000000000000000000000DEAD": `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
contract ProxyRugToken {
    ...
    function setImplementation(address _impl) external onlyOwner { implementation = _impl; }
